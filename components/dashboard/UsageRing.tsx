...
// Smoothly count from prev → next value
...
export default function UsageRing({ used, limit }: Props) {
  ...
  // Animate arc percentage with JS (same rAF approach)
  ...
  const labelColor =
    rawPct >= 90 ? "text-rose-400"
    : rawPct >= 70 ? "text-orange-400"
    : "text-white";

  // Flash effect when `used` changes
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl flex flex-col items-center justify-center gap-4">
      ...
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <svg width={size} height={size} className="-rotate-90">
          ...
          />
          {/* Arc — offset driven by rAF, color transition via CSS */}
          <circle
            cx={size / 2} cy={size / 2} r={radius}
            fill="none"
            stroke={ringColor}
        ...
        <div className="w-full h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
          <div
            ...
            style={{
              ...
              // width animated by rAF via arcPct
            }}
          />
        </div>
