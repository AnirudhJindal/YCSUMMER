...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className={`inline-flex px-3 py-1 rounded-full text-xs border ${colors}`}>
      {type}
    </span>
  );
}
...
export default function TokensPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      await api("/api/token", {
        method: "DELETE",
        body: JSON.stringify({ tokenId: deleteTarget }),
      });
  ...
  if (loading) return <div className="text-white/40">Loading tokens...</div>;
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-white/40 mt-2">
            Every masked value stored in your vault.
          </p>
        </div>
        <div className="px-4 py-2 rounded-full border border-white/10 bg-white/5 text-white/40 text-sm">
      ...
      {tokens.length > 0 && (
        <div className="flex items-center gap-3 mb-6">
          <input
            ...
            className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-violet-400/40 text-white/70 placeholder:text-white/20 w-64"
          />
            ...
            {allTypes.map((type) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs transition-all border ${
                  filterType === type
                    ? "bg-violet-500/20 border-violet-400/20 text-violet-300"
      ...
      {tokens.length === 0 ? (
        <div className="glass rounded-3xl p-16 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-white/40 text-sm">
            Tokens appear here once you start making mask requests via your API.
          </p>
        </div>
      ) : filtered.length === 0 ? (
              ...
              {filtered.map((t) => (
                ...
                >
                  <td className="px-6 py-5">
                    <code className="text-xs text-white/50 bg-white/5 px-2 py-1 rounded-lg">
                      {t.token}
                    </code>
                  </td>
                  <td className="px-6 py-5">
      ...
      {deleteTarget && (
        ...
        >
          <p className="text-white/40 text-sm mb-6">
            This token will be permanently deleted. Any app trying to unmask it
            will get an error. This cannot be undone.
          </p>
