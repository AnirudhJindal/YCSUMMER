...
const TIERS: Tier[] = [
  {
    ...
    features: [
      ...
      {
        label: "Shared vault infrastructure",
        note: "multi-tenant",
        disabled: false,
      },
  ...
  },
  {
    ...
    cta: "Let's talk",
    ctaHref:
      "mailto:vaultyin@gmail.com?subject=Enterprise%20Plan%20Inquiry&body=Hi%2C%20I%27m%20interested%20in%20the%20Enterprise%20plan.",
    highlight: true,
    database: true,
    features: [
...
export default function PricingPage() {
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-20 sm:pt-36 pb-16 sm:pb-28">
        {/* Header */}
        <motion.div
        ...
        >
          <p className="text-xs sm:text-sm tracking-[0.25em] uppercase text-white/30 mb-4 sm:mb-5">
            Pricing
          </p>

          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-light tracking-[-0.04em] leading-[1.05] mb-4 sm:mb-6">
            Pay for what you mask.
            <br />
            <span className="text-white/30">Nothing more.</span>
          </h1>

          <p className="text-white/40 text-base sm:text-xl max-w-lg mx-auto leading-relaxed tracking-[-0.02em]">
            Every plan gives you the same core API — masking,
            unmasking, deep JSON — the tiers differ in scale and
            isolation.
          </p>
        </motion.div>
          ...
          {TIERS.map((tier) => {
            return (
              <motion.div
                ...
                className={[
                  "relative rounded-2xl p-6 sm:p-8 flex flex-col transition-colors duration-300",
                  tier.highlight
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span
                    className={[
                      "text-xs sm:text-sm font-medium tracking-[0.15em] uppercase",
                      tier.highlight
                        ? "text-black/40"
                        : "text-white/30",
                    ].join(" ")}
                ...
                <div className="mb-4">
                  {tier.price !== null ? (
                    <div className="flex items-end gap-1.5">
                      <span
                        className={[
                          "text-5xl sm:text-6xl font-light tracking-[-0.04em]",
                          tier.highlight
                            ? "text-black"
                            : "text-white",
                        ].join(" ")}
                    ...
                    <div
                      className={[
                        "text-4xl sm:text-5xl font-light tracking-[-0.04em]",
                        tier.highlight
                          ? "text-black"
                          : "text-white",
                      ].join(" ")}
                  ...
                  <p
                    className={[
                      "text-sm sm:text-base mt-2 tracking-[-0.01em]",
                      tier.highlight
                        ? "text-black/50"
                        : "text-white/[0.35]",
                    ].join(" ")}
                ...
                <a
                  href={tier.ctaHref}
                  className={[
                    "block text-center py-3 rounded-xl text-sm sm:text-base font-medium tracking-[-0.01em] transition-colors mb-6 sm:mb-8",
                    tier.highlight
                ...
                <ul className="flex flex-col gap-3 sm:gap-3.5">
                  {tier.features.map((f) => (
                    <li
                      key={f.label}
                      className={[
                        "flex items-start gap-2 sm:gap-2.5 text-sm sm:text-base tracking-[-0.01em]",
                    ...
                    >
                      <span
                        aria-hidden="true"
                        className={[
                          "mt-0.5 text-sm sm:text-base leading-none shrink-0",
                          f.disabled
                            ? tier.highlight
                              ? "text-black/25"
                              : "text-white/20"
        ...
        <motion.p
          ...
          initial="hidden"
          animate="show"
          className="text-center text-white/20 text-xs sm:text-sm mt-10 sm:mt-14 tracking-[-0.01em] px-4"
        >
          All plans include the same core API. Overages on Free are
          hard-stopped; Pro overages billed at $0.05 / 1k requests.
          <br />
          Need a volume deal?{" "}
          <a
